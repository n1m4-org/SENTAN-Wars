...
void RankingView::Initialize(const Config& config)
{
    ...
    for (size_t i = 0; i < kSlotCount_; ++i)
    {
        ...
        sprite->Initialize(Path::Image::Numbers[i + 1], { config_.leftX, config_.slotCenterY[i] }, MedalColor(i), { 0.0f, 0.5f });
...
void RankingView::ShowStatic(const std::array<float, 3>& entries)
{
    mode_ = Mode::Static;
    insertRank_ = -1;
    animTimer_ = 0.0f;
...
void RankingView::StartInsert(const std::array<float, 3>& oldEntries, const std::array<float, 3>& newEntries, int insertRank)
{
    mode_ = Mode::Sliding;
...
void RankingView::SkipToEnd(const std::array<float, 3>& newEntries)
{
