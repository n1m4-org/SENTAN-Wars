...
class BossLeapAttackComponent : public Component
{
public:
    /// <summary>
    /// 
    ...
    explicit BossLeapAttackComponent(Hagine::WorldTransform* transform, float* attackRange, Hagine::Vector3* target, float* radius)
...
private:
    ...
    GameParameter(float, attackTimer_, 5.0f);
    GameParameter(float, attackTime_, 3.5f);
    GameParameter(float, coolTime_, 11.5f);
    GameParameter(float, jumpHeight_, 12.0f);
};
