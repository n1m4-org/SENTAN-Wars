...
class RamAttackComponent : public Component
{
public:
    /// 必要な物はコンストラクタで受け取る
    ...
    explicit RamAttackComponent(Hagine::WorldTransform* transform, float* attackRange, Hagine::Vector3* target, float* radius)
...
private:
    ...
    GameParameter(float, coolTime_, 1.5f);
};
